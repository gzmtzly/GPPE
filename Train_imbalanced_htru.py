...
### train data
train_data_pulsar_path = np.load("./file_info_trans64/pulsar_path_trans64.npy")[:360]
# train_data_pulsar_path = np.array(list(train_data_pulsar_path) * 75)
train_data_rfi_path = np.load("./file_info_trans64/rfi_path_trans64.npy")[:27000]

print('train_data_pulsar_path:', len(train_data_pulsar_path))
print('train_data_rfi_path:', len(train_data_rfi_path))

### test data
test_data_pulsar_path = np.load("./file_info_trans64/pulsar_path_trans64.npy")[600:]
test_data_rfi_path = np.load("./file_info_trans64/rfi_path_trans64.npy")[45000:]
...
CONV = Model_htru.Conv().cuda()

optimer = torch.optim.Adam(CONV.parameters(), lr=0.5, betas=(0.5, 0.999))   # FTL在lr=0.0001下才能训练出结果
